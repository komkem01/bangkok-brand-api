# AI Agent Instructions - bangkok-brand Storage Service
...
func New(conf *config.Config[Config], dependencies...) *Module {
    tracer := otel.Tracer("bangkok-brand.service_name.module_name")
...
```go
import "log/slog"
import "bangkok-brand/internal/log"
...
```go
import "bangkok-brand/internal/provider"
...
```bash
# Application
APP_NAME=bangkok-brand-storage-service
